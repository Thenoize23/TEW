/* Bounce Check: loudness, dynamics, peak, mono and spectral balance of a bounce,
   ...
   again) and, optionally, an element #bouncecheck-root with:
...
function showLimit(){
  paintQuota();
  try{ gtag('event','bouncecheck_limit',{page: CFG.page, used: checksUsed()}); }catch(_){}
  if (limitEl) limitEl.scrollIntoView({behavior: 'smooth', block: 'center'});
}
if ($('limit-cta')) $('limit-cta').addEventListener('click', function(e){
  try{ gtag('event','paid_intent',{plan: 'starter', source: 'bouncecheck_limit', page: CFG.page}); }catch(_){}
  ...
  if (window._tewOpenWaitlist){
    e.preventDefault();
    if (!window._tewSrc) window._tewSrc = 'bouncecheck-limit';
...
function render(lufs, pk, sp, buf){
  ...
  $('n-cta').href = CFG.app + '?signup=1&src=bouncecheck&genre=' + encodeURIComponent(label) +
